# encoding: UTF-8
puts "##################################################################################################"
puts "################################## Export sources to ICCU     ####################################"
puts "##################################################################################################"
puts ""
require_relative "../lib/maintenance"
NAMESPACE={'marc' => "http://www.loc.gov/MARC21/slim"}
sources = Source.where(:wf_stage => 1).where('lib_siglum like ?', "I-%").where.not(wf_owner: 268).where("record_type = ? or record_type = ? or record_type = ? or record_type = ?", 1,2,4,6).order(:id).pluck(:id)
filename = "/tmp/iccu.xml"
