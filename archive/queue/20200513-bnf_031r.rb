# encoding: UTF-8
puts "##################################################################################################"
puts "################    ISSUE tasks: Cleanup 031r values                       #######################"
...
terms = YAML.load_file("#{Rails.root}/housekeeping/maintenance/20200513-bnf_031r.yml")
...
process = lambda { |record|
  modified = false
  marc = record.marc
  marc.each_by_tag("031") do |n|
    n.each_by_tag("r") do |sf|
      old_content = sf.content
      new_content = replace_all(sf.content,terms)
      if new_content != old_content
        modified = true
        maintenance.logger.info("#{maintenance.host}: Source ##{record.id} 031$r: '#{old_content.yellow}' => '#{new_content.green}'")
