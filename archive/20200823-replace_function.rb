...
size = Source.find_by_sql("SELECT * FROM sources where wf_owner=268 and wf_stage = 0 and marc_source like '%=700%'").size
logger = Logger.new("#{File.dirname($0)}/log/#{File.basename($0, '.rb')}.log")
bar = ProgressBar.new(size)

(0..(size / PACKETS )).each do |packet|
  GC.start
  sources = Source.find_by_sql("SELECT * FROM sources where wf_owner=268 and wf_stage = 0 and marc_source like '%=700%' order by id LIMIT #{PACKETS} offset #{PACKETS * packet}")
  ...
  sources.each do |record| 
    ...
    if modified
      ...
      logger.info("#{host}: ICCU #{record.id} change 700$4 function")
    end
  end
end
